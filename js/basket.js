  ...
  fetch("http://localhost:3000/api/furniture")
    ...
    .then((produit) => {
      ...
      localStorage.setItem(
        "requiredtosendserver",
        JSON.stringify(responseElement)
      );
...
function displayBasketPage() {
  ...
  for (let i = 0; i < obsJsonlocalstorage.length; i++) {
    for (
      let j = 0;
      j < obsJsonlocalstorage[i].numberofoptionarticle.length;
      j++
    ) {
    ...
    addArticle += `<div id="ctn${
      obsJsonlocalstorage[i].namearticle
    }" class="basketpage-ctnarticle3">
            <div id="name${
              obsJsonlocalstorage[i].namearticle
            }" class="listarticle">
        ...
        <div id="alloptionarticle${
          obsJsonlocalstorage[i].namearticle
        }" class="alloptionarticle">
            ${addOptionArticle}
        </div>`;
    addOptionArticle = "";
  }
  ...
  function addOneItemToStorage(p) {
    p.addEventListener("click", function (e) {
      ...
      for (let j = 0; j < obsJsonlocalstorage.length; j++) {
        for (
          let k = 0;
          k < obsJsonlocalstorage[j].numberofoptionarticle.length;
          k++
        ) {
          if (
            getDomId ==
            obsJsonlocalstorage[j].namearticle +
              obsJsonlocalstorage[j].numberofoptionarticle[k].optionarticle
          ) {
            let newprice =
              obsJsonlocalstorage[j].pricearticle /
              obsJsonlocalstorage[j].quantityarticle;
            obsJsonlocalstorage[j].pricearticle += newprice;
            obsJsonlocalstorage[j].numberofoptionarticle[k]
              .quantityoptionarticle++;
            obsJsonlocalstorage[j].quantityarticle++;
            localStorage.setItem(
              "session",
              JSON.stringify(obsJsonlocalstorage)
            );
            document.getElementById(
              ...
              "</p>";
            document.getElementById(
              "quantity" + obsJsonlocalstorage[j].namearticle
            ).innerHTML =
              "<p>" + obsJsonlocalstorage[j].quantityarticle + "</p>";
            document.getElementById(
              "price" + obsJsonlocalstorage[j].namearticle
            ).innerHTML =
              "<p>" + obsJsonlocalstorage[j].pricearticle / 1000 + "£<p>";
            break;
          }
        }
      }
        ...
        for (
          ...
          if (
            ...
            if (
              ...
              let parentOfElementRemove = document.getElementById(
                "alloptionarticle" + obsJsonlocalstorage[j].namearticle
              );
              document.getElementById(
                "quantity" + obsJsonlocalstorage[j].namearticle
              ).innerHTML =
              ...
              let indextoDeleteOption = obsJsonlocalstorage[
                j
              ].numberofoptionarticle
                .map((e) => e.optionarticle)
                .indexOf(
                  obsJsonlocalstorage[j].numberofoptionarticle[k].optionarticle
                );
              obsJsonlocalstorage[j].numberofoptionarticle.splice(
                indextoDeleteOption,
                1
              );
              localStorage.setItem(
              ...
              if (obsJsonlocalstorage[j].numberofoptionarticle.length == 0) {
                let articleElementRemove = document.getElementById(
                  "ctn" + obsJsonlocalstorage[j].namearticle
                );
                let articleElementRemove2 = document.getElementById(
                  "alloptionarticle" + obsJsonlocalstorage[j].namearticle
                );
                let articleparentOfElementRemove = document.getElementById(
                  "articlebasketpage-ctn2"
                );
                ...
                let indextoDeleteArticle = obsJsonlocalstorage
                  .map((e) => e.namearticle)
                  .indexOf(obsJsonlocalstorage[j].namearticle);
      ...
      for (let j = 0; j < obsJsonlocalstorage.length; j++) {
        for (
          ...
          if (
            ...
            obsJsonlocalstorage[j].pricearticle -= newprice;
            obsJsonlocalstorage[j].quantityarticle -=
              obsJsonlocalstorage[j].numberofoptionarticle[
                k
              ].quantityoptionarticle;
            obsJsonlocalstorage[j].numberofoptionarticle[
              k
